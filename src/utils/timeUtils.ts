...
export const extractTimePassedFromText = (
  text: string,
  ...
  const rangeMatch = lowerText.match(
    /từ\s*(\d+)\s*(giờ|h)?\s*(sáng|chiều|tối|đêm)?\s*đến\s*(\d+)\s*(giờ|h)?\s*(sáng|chiều|tối|đêm)?/i,
  );
  if (rangeMatch) {
  ...
  // --- 3. Dạng Mốc (Until) ---
  const milestoneHours: { [key: string]: number } = {
    sáng: 6,
    trưa: 12,
    chiều: 14,
    tối: 18,
...
export const shouldWeatherUpdate = (
  timePassed: TimePassed,
  oldTime: WorldTime,
  newTime: WorldTime,
): boolean => {
