...
interface PartialContext {
  ...
  activeQuests: Quest[]; // Chỉ chứa Quest chưa hoàn thành
  completedQuestsSummary: string[]; // Danh sách tên các Quest đã hoàn thành
...
export function selectRelevantContext(
  ...
  if (gameState.currentLocationId) {
    ...
    (gameState.encounteredNPCs || []).forEach((npc) => {
      if (
        npc.locationId?.toLowerCase() === lowerCaseLocation &&
        !includedNpcNames.has(npc.name.toLowerCase())
      ) {
    ...
    [
      ...
      if (
        entity.locationId?.toLowerCase() === lowerCaseLocation &&
        !includedEntityNames.has(entity.name.toLowerCase())
      ) {
  ...
  const allSearchableEntities = [
    ...
    ...(gameState.quests || []), // Vẫn quét tất cả quest để tìm theo từ khóa nếu cần
    ...(gameState.inventory || []),
  ...
  allSearchableEntities.forEach((entity) => {
    if (entity.name && lowerCaseInput.includes(entity.name.toLowerCase())) {
      ...
      if (
        ...
        "description" in entity &&
        !("value" in entity) &&
        !includedSkillNames.has(entityNameLower)
      ) {
        // Đây là Skill
        if (
          ...
          partialContext.characterSkills = [
            ...(partialContext.characterSkills || []),
            entity as { name: string; description: string },
          ];
        }
  ...
  (gameState.encounteredNPCs || []).forEach((npc) => {
    if (npc.memoryFlags) {
      const hasImportantFlag = Object.keys(npc.memoryFlags).some((flag) =>
        ["đã kết hôn", "kẻ thù truyền kiếp"].includes(flag),
      );
