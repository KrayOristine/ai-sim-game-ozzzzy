...
export const obfuscateText = (text: string): string => {
  ...
  const sensitiveKeywords = [
    ...
    "chém",
    "hiếp",
    "dâm", // Thêm các từ khóa bạo lực và nhạy cảm
  ];
...
export const processNarration = (narration: string): string => {
  ...
  cleanedText = cleanedText.replace(
    /<\/?(?!(?:entity|important|status|exp|thought)\b)[a-zA-Z][^>]*>/gi,
    "",
  );
 ...
 */
export function detectEntityTypeAndCategory(name: string): {
  type: CoreEntityType | null;
  ...
  // Thế lực
  if (
    /\b(bang|phái|môn|giáo|tông|gia|tộc|cung|điện|lâu|các)\b/.test(lowerName) &&
    !/\b(công pháp|tâm pháp)\b/.test(lowerName)
  ) {
  ...
  // Địa điểm
  if (
    /\b(thành|làng|hang|động|sông|núi|rừng|thung lũng|quốc|cốc|viện|phủ|biển|hồ|đảo)\b/.test(
      lowerName,
    )
  ) {
