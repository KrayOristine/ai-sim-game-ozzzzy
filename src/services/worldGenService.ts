...
export async function generateWorldFromIdea(
  idea: string,
  ...
  if (backgroundKnowledge && backgroundKnowledge.length > 0) {
    const totalKnowledgeSize = backgroundKnowledge.reduce(
    ...
    if (hasDataset && totalKnowledgeSize > KNOWLEDGE_SIZE_THRESHOLD) {
      ...
      knowledgeForGeneration = await retrieveRelevantKnowledgeChunks(
        idea,
        backgroundKnowledge,
        7,
        queryEmbedding,
  ...
  const { prompt, schema, creativeCallConfig } = getGenerateWorldFromIdeaPrompt(
    idea,
...
export async function generateFanfictionWorld(
  idea: string,
  enableMilestoneSystem: boolean,
  backgroundKnowledge?: { name: string; content: string }[],
): Promise<WorldConfig> {
...
export const generateEntityPersonality = (
  config: WorldConfig,
  entity: InitialEntity,
): Promise<string> => {
...
export const generateEntityDescription = (
  config: WorldConfig,
  entity: InitialEntity,
): Promise<string> => {
...
export async function generateFandomSummary(
  workName: string,
  authorName?: string,
): Promise<string> {
  ...
  if (result.includes("WORK_NOT_FOUND")) {
    throw new Error(
      `Không tìm thấy thông tin chi tiết về tác phẩm "${workName}"${authorName ? ` (tác giả: ${authorName})` : ""}. Vui lòng kiểm tra lại tên.`,
    );
  }
  return result;
}
...
export async function generateFandomGenesis(
  summaryContent: string,
  arcName: string,
  workName: string,
  authorName?: string,
  ...
  if (result.trim() === "ARC_NOT_FOUND") {
    throw new Error(
      `Không tìm thấy thông tin về Arc "${arcName}" trong bản tóm tắt được cung cấp.`,
    );
  }
  return result;
}
