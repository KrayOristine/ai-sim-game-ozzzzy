...
import { GoogleGenAI, HarmCategory, HarmBlockThreshold, type SafetySetting, ThinkingConfig, ThinkingLevel } from "@google/genai";
import { getSettings } from '#/services/settingsService';
import { AiPerformanceSettings, SafetySettingsConfig } from '#/types';
import { DEFAULT_AI_PERFORMANCE_SETTINGS, AIModel } from '#/const';
...
export async function generate(prompt: string, systemInstruction?: string, retryCount: number = 0, modelOverride?: AIModel): Promise<string> {
    ...
    const perfSettings: AiPerformanceSettings = aiPerformanceSettings || DEFAULT_AI_PERFORMANCE_SETTINGS;

    // Ưu tiên sử dụng model từ tham số override, sau đó mới đến cài đặt
    let selectedModel = modelOverride || perfSettings.selectedModel || 'gemini-3.1-flash-lite-preview';
...
export async function generateJson<T>(prompt: string, schema: any, systemInstruction?: string, model: AIModel = AIModel.Gemini3d1FlashLite, overrideConfig?: Partial<AiPerformanceSettings>, retryCount: number = 0): Promise<T> {
    const { safetySettings, aiPerformanceSettings } = getSettings();
    const activeSafetySettings = safetySettings.enabled ? safetySettings.settings : UNRESTRICTED_SAFETY_SETTINGS;
