...
async function getInjectedMemories(
  gameState: GameState,
  ...
  if (history.length <= 10) {
    if (DEBUG_MODE) {
      console.log(
        `%c[CONDITIONAL RAG]`,
        "color: orange;",
        `History too short (${history.length} <= 10). Skipping Embedding & RAG.`,
      );
    }
    return { memories: "" };
  }
  ...
  if (involvedNpcsInAction.length > 0 && npcDossiers) {
    if (DEBUG_MODE) {
      console.log(
        `%c[METHOD 1: NPC DOSSIER]`,
        "color: yellow; font-weight: bold;",
        `NPCs detected: ${involvedNpcsInAction.join(", ")}`,
      );
    }
    let dossierContent = "";
  ...
  try {
    ...
    const searchableTurnVectors = allTurnVectors.filter(
      (v) => v.turnIndex < history.length - NUM_RECENT_TURNS,
    );

    if (searchableTurnVectors.length > 0) {
      const vectorRankedTurns = searchableTurnVectors
        .map((vector) => ({
          id: vector.turnIndex,
      ...
      const keywordRankedTurns = searchableTurnVectors
        .map((vector) => ({
          id: vector.turnIndex,
  ...
  try {
    ...
    if (allSummaryVectors.length > 0) {
      const vectorRankedSummaries = allSummaryVectors
        .map((vector) => ({
        ...
        .sort((a, b) => b.score - a.score);
      const keywordRankedSummaries = allSummaryVectors
        .map((vector) => ({
          id: vector.summaryIndex,
          score: calculateKeywordScore(ragQueryText, vector.content),
          data: vector,
        ...
        .sort((a, b) => b.score - a.score);
      const fusedSummaryResults = reciprocalRankFusion([
        vectorRankedSummaries,
        keywordRankedSummaries,
      ]);
  ...
  if (DEBUG_MODE) {
    console.log(
      `%c[FOUND TURNS: ${foundTurnsCount}]`,
      "color: lightblue;",
      relevantPastTurns || "Không có.",
    );
    console.log(
      `%c[FOUND MEMORIES: ${foundSummariesCount}]`,
      "color: lightblue;",
      relevantMemories || "Không có.",
    );
  }
...
export const getNextTurn = async (
  ...
  const previousContent = previousTurn
    ? `${previousTurn.type === "action" ? "Người chơi" : "AI"}: ${previousTurn.content.replace(/<[^>]*>/g, "").substring(0, 200)}...`
    : "";
  ...
  if (
    ...
    bufferEmbeddings.length === pendingItems.length &&
    gameState.worldId
  ) {
    ...
    if (DEBUG_MODE) {
      console.log(
        `%c[PIGGYBACK SAVED]`,
        "color: #34d399;",
        `Saved ${pendingItems.length} items from buffer to DB.`,
      );
    }
  }
  ...
  const { memories: injectedMemories } = await getInjectedMemories(
    gameState,
    queryEmbedding,
    ragQueryText,
  );
  ...
  if (
    ...
    relevantKnowledge = await ragService.retrieveRelevantKnowledge(
      ragQueryText,
      worldConfig.backgroundKnowledge,
      3,
      queryEmbedding,
    ...
    try {
      ...
      if (DEBUG_MODE) {
        console.log(
          `%c[AUTO SUMMARY]`,
          "color: #a855f7;",
          `Generated summary for last ${ragSettings.summaryFrequency} turns.`,
        );
      }
    } catch (e) {
