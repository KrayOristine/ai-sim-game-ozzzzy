      ...
      switch (event.oldVersion) {
        case 0:
          if (!dbInstance.objectStoreNames.contains(SAVES_STORE_NAME)) {
            dbInstance.createObjectStore(SAVES_STORE_NAME, {
              keyPath: "saveId",
            });
          }
        case 1:
        ...
        case 5:
          ...
          if (!dbInstance.objectStoreNames.contains(GRAPH_NODES_STORE_NAME)) {
            const store = dbInstance.createObjectStore(GRAPH_NODES_STORE_NAME, {
              keyPath: ["worldId", "id"],
            });
            store.createIndex("worldId", "worldId", { unique: false });
          }
          if (!dbInstance.objectStoreNames.contains(GRAPH_EDGES_STORE_NAME)) {
            const store = dbInstance.createObjectStore(GRAPH_EDGES_STORE_NAME, {
  ...
  return new Promise((resolve, reject) => {
    const transaction = db.transaction(
      [
        ...
        SUMMARY_VECTORS_STORE_NAME,
        ENTITY_VECTORS_STORE_NAME,
        GRAPH_NODES_STORE_NAME,
        GRAPH_EDGES_STORE_NAME,
      ],
      "readwrite",
    );
