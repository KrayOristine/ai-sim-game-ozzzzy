...
export const updateDynamicStateFromNarration = async (gameState: GameState, lastNarration: string): Promise<DynamicStateUpdateResponse | null> => {
    ...
    try {
        return await generateJson<DynamicStateUpdateResponse>(prompt, schema, undefined, AIModel.Gemini3d1FlashLite, analyticalCallConfig);
    } catch (error) {
...
export const updateEncyclopediaEntriesFromNarration = async (gameState: GameState, lastNarration: string): Promise<EncyclopediaEntriesUpdateResponse | null> => {
    ...
    try {
        return await generateJson<EncyclopediaEntriesUpdateResponse>(prompt, schema, undefined, AIModel.Gemini3d1FlashLite, analyticalCallConfig);
    } catch (error) {
...
export const updateCharacterStateFromNarration = async (gameState: GameState, lastNarration: string): Promise<CharacterStateUpdateResponse | null> => {
    ...
    try {
        const response = await generateJson<CharacterStateUpdateResponse>(prompt, schema, undefined, AIModel.Gemini3d1FlashLite, analyticalCallConfig);
...
export const normalizeCategoriesWithAI = async (allEntities: { name: string, customCategory?: string }[]): Promise<{ oldCategory: string, newCategory: string }[]> => {
    ...
    try {
        const result = await generateJson<{ normalizationMappings: { oldCategory: string, newCategory: string }[] }>(prompt, schema, undefined, AIModel.Gemini3d1FlashLite, analyticalCallConfig);
...
export const deduplicateEntitiesInCategoryWithAI = async (entities: { name: string; id: string }[]): Promise<{ idToDelete: string, idToKeep: string }[]> => {
    ...
    try {
        const result = await generateJson<{ deduplicationPairs: { idToDelete: string, idToKeep: string }[] }>(prompt, schema, undefined, AIModel.Gemini3d1FlashLite, analyticalCallConfig);
