...
export async function generateWorldFromIdea(idea: string, enableMilestoneSystem: boolean, backgroundKnowledge?: {name: string, content: string}[]): Promise<WorldConfig> {
    ...
    return generateJson<WorldConfig>(prompt, schema, undefined, AIModel.Gemini3d1FlashLite, creativeCallConfig, 0);
}
...
export async function extractArcListFromSummary(summaryContent: string): Promise<string[]> {
    ...
    const result = await generateJson<{ arcs: string[] }>(prompt, schema, undefined, AIModel.Gemini3d1FlashLite, undefined, 0);
    return result.arcs || [];
}
...
export const generateEntityInfoOnTheFly = (
    ...
    return generateJson<InitialEntity>(prompt, schema, undefined, AIModel.Gemini3d1FlashLite, creativeCallConfig, 2);
};
