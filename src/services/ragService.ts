...
export async function compressNpcDossier(
  gameState: GameState,
  npcName: string,
): Promise<GameState> {
  ...
  try {
    ...
    if (newArchivedFacts.length > 0) {
      ...
      if (DEBUG_MODE) {
        console.log(
          `[DOSSIER COMPRESSION] Đã tóm tắt ${turnsToSummarizeCount} lượt tương tác cho ${npcName}. Thêm ${newArchivedFacts.length} sự kiện vào kho lưu trữ.`,
        );
      }
...
export async function retrieveRelevantKnowledgeChunks(
  context: string,
  allKnowledge: { name: string; content: string }[],
  topK: number,
    ...
    try {
      ...
      for (const file of datasetFiles) {
        try {
          ...
          if (
            dataset.metadata?.embeddingModel &&
            dataset.chunks?.every((c) => Array.isArray(c.embedding))
          ) {
          ...
          console.error(
            `Could not parse or process dataset file for vector search: ${file.name}`,
            e,
          );
        }
      }
...
export async function retrieveRelevantKnowledge(
  ...
  const selectedKnowledgeFiles = await retrieveRelevantKnowledgeChunks(
    context,
    allKnowledge,
    topK,
    queryEmbedding,
  ...
  const hasDetailFiles = selectedKnowledgeFiles.some((f) =>
    f.name.startsWith("Chi_tiet_lien_quan_"),
  );
...
export async function distillKnowledgeForWorldCreation(
  ...
  const createAndSaveEmbeddedDataset = async () => {
    try {
      ...
      const baseName = dataset.metadata.sourceName
        .replace(/\.txt$/i, "")
        .replace(/[\s/\\?%*:|"<>]/g, "_");
  ...
  for (let i = 0; i < textChunks.length; i += BATCH_SIZE_DISTILL) {
    ...
    const batchPromises = batch.map((chunk) => {
      ...
      return generate(prompt, undefined, 0, AIModel.Gemini3d1FlashLite);
    });
    ...
    try {
      ...
      throw new Error(
        `Lỗi khi đang chắt lọc kiến thức nền. Vui lòng thử lại. Lỗi chi tiết: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
 ...
 */
export async function processVectorUpdates(
  ...
  try {
    ...
    if (embeddings.length === updates.length) {
      ...
      if (DEBUG_MODE) {
        console.log(
          `[RAG SERVICE] Đã xử lý và lưu trữ thành công ${newVectors.length} bản cập nhật vector cho worldId: ${worldId}.`,
        );
      }
    }
  } catch (error) {
...
export async function sanitizeTextForAIContext(
  text: string,
  worldConfig: WorldConfig,
): Promise<string> {
  if (!text.trim()) {
