...
${config.enableStatsSystem ? `[PLAYER_STATS_INIT: name="Sinh Lực", value=100, maxValue=100, isPercentage=true, description="Sức sống", hasLimit=true] (Sử dụng cho MỖI chỉ số)` : ""}
...
export const getStartGamePrompt = (config: WorldConfig) => {
  ...
  const tagInstructions = getTagInstructions([], config); // Start game không có custom categories
  ...
  const nsfwPayload = buildNsfwPayload(config, getSettings().safetySettings);
...
${backgroundKnowledgeString.join("\n")}`; // Thêm phần kiến thức đã lọc vào context
...
export const getNextTurnPrompt = async (
  ...
  if (isSafetyBypassMode) {
    ...
    recentHistoryString = recentHistoryTurns
      .map(
        (turn) =>
  ...
  if (
    fullContext.encounteredNPCs &&
    Array.isArray(fullContext.encounteredNPCs) &&
    fullContext.encounteredNPCs.length > 0
  ) {
  ...
  if (physicalStateContext.length > 0) {
    worldStateContextParts.push(
      `--- DỮ LIỆU CỨNG VỀ TRẠNG THÁI VẬT LÝ & CẢM XÚC ---\n${physicalStateContext.join("\n")}\n--- KẾT THÚC DỮ LIỆU CỨNG ---`,
    );
  }
  ...
  const coreInfo = {
    worldConfig: {
      ...
      coreRules: worldConfig.coreRules,
      temporaryRules: worldConfig.temporaryRules,
      aiResponseLength: worldConfig.aiResponseLength,
    },
    character: {
      name: character.name,
