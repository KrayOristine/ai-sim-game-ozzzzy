...
export const getGenerateCharacterSkillsPrompt = (config: WorldConfig) => {
  ...
  const skillSchema = {
    ...
    properties: {
      ...
      description: {
        type: Type.STRING,
        description: "Mô tả ngắn gọn về kỹ năng.",
      },
    },
...
export const getGenerateCharacterStatsPrompt = (config: WorldConfig) => {
  ...
  const schema = {
    type: Type.ARRAY,
    description:
      "Một danh sách từ 2-4 chỉ số nhân vật bổ sung (không bao gồm Sinh Lực và Thể Lực đã có sẵn) phù hợp với thể loại và tiểu sử.",
    items: statSchema,
  };
...
export const getGenerateSingleSkillPrompt = (
  ...
  const schema = {
    ...
    properties: {
      ...
      description: {
        type: Type.STRING,
        description: "Mô tả ngắn gọn, hấp dẫn về kỹ năng.",
      },
    },
    required: ["name", "description"],
  };
...
export const getGenerateSingleMilestonePrompt = (
  config: WorldConfig,
