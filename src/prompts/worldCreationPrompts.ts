...
const getCleanContent = (content: string, filterKeyword?: string): string => {
  if (isFandomDataset(content)) {
    if (filterKeyword) {
      // Sàng lọc thông minh nếu có từ khóa (ý tưởng, tên tác phẩm...)
      return filterDatasetChunks(content, [filterKeyword], 30);
    } else {
      // Mặc định lấy sạch nhưng chỉ phần đầu để tránh lỗi
      return filterDatasetChunks(content, [], 20);
    }
  }
  return content;
};

const buildWorldCreationKnowledgePrompt = (
  knowledge?: { name: string; content: string }[],
  filterKeyword?: string,
): string => {
  if (!knowledge || knowledge.length === 0) return "";

  let prompt = "\n\n--- KIẾN THỨC NỀN (TÀI LIỆU THAM KHẢO ĐÃ SÀNG LỌC) ---\n";
  prompt +=
    "Đây là các tài liệu tham khảo được chọn lọc. Hãy sử dụng chúng làm cơ sở cốt lõi để kiến tạo thế giới.\n\n";

  prompt += knowledge
    .map(
      (file) =>
        `--- NGUỒN: ${file.name} ---\n${getCleanContent(file.content, filterKeyword)}\n--- KẾT THÚC NGUỒN ---`,
    )
    .join("\n\n");
...
export const buildBackgroundKnowledgePrompt = (
  knowledge?: { name: string; content: string }[],
  hasDetailFiles: boolean = false,
): string => {
  if (!knowledge || knowledge.length === 0) return "";
  ...
  if (hasDetailFiles) {
    prompt +=
      "Sử dụng các thông tin sau làm kiến thức nền. TÓM TẮT TỔNG QUAN luôn được cung cấp. CHI TIẾT LIÊN QUAN được chọn lọc và cung cấp dựa trên diễn biến gần đây. Hãy ưu tiên sử dụng chúng để làm rõ bối cảnh khi cần.\n";
  } else {
    prompt +=
      "Sử dụng các thông tin sau làm kiến thức nền. ƯU TIÊN đọc TÓM TẮT TỔNG QUAN trước, sau đó dùng các tệp PHÂN TÍCH CHI TIẾT để làm rõ khi cần.\n";
  }

  if (summaries.length > 0) {
    prompt += "\n### TÓM TẮT TỔNG QUAN ###\n";
    prompt += summaries
      .map((s) => `--- NGUỒN: ${s.name} ---\n${getCleanContent(s.content)}`)
      .join("\n\n");
  }
...
const getWorldCreationSchema = (generateMilestones: boolean) => {
  ...
  const entitySchema = {
    ...
    properties: {
      ...
      type: {
        ...
        description: "Loại cốt lõi của thực thể.",
      },
      customCategory: {
        type: Type.STRING,
        description: "Phân loại chi tiết hơn do người dùng hoặc AI tạo.",
      },
      personality: {
        type: Type.STRING,
        description:
      ...
      description: {
        type: Type.STRING,
        description: "Mô tả chi tiết về thực thể.",
      },
      tags: {
        type: Type.ARRAY,
        items: { type: Type.STRING },
        description:
          "Một danh sách các tags mô tả ngắn gọn (VD: 'Quan trọng', 'Cổ đại', 'Học thuật') để phân loại thực thể.",
      },
    },
  ...
  const statSchema = {
    ...
    properties: {
      ...
      description: {
        type: Type.STRING,
        description: "Mô tả ngắn gọn về công dụng của chỉ số này trong game.",
      },
      hasLimit: {
        type: Type.BOOLEAN,
        description:
  ...
  const characterProperties: any = {
    ...
    personality: {
      type: Type.STRING,
      description: "Luôn đặt giá trị này là 'Tuỳ chỉnh'.",
    },
    customPersonality: {
      type: Type.STRING,
      description:
        "Một danh sách các tính từ ngắn gọn mô tả tính cách, ngăn cách bằng dấu phẩy (VD: Lạnh lùng, quyết đoán, thông minh). Không viết thành câu.",
    },
    gender: {
      type: Type.STRING,
      enum: GENDER_OPTIONS,
      description: "Giới tính của nhân vật.",
    },
    ...
    stats: {
      type: Type.ARRAY,
      description:
        "Danh sách các chỉ số của nhân vật, luôn bao gồm Sinh Lực và Thể Lực, và có thể thêm các chỉ số khác phù hợp với thể loại.",
      items: statSchema,
    },
    motivation: {
      type: Type.STRING,
      description: "Mục tiêu hoặc động lực chính của nhân vật.",
    },
  };
  ...
  if (generateMilestones) {
    characterProperties.milestones = {
      type: Type.ARRAY,
      description:
        "Danh sách các cột mốc (chỉ số dạng chữ) phù hợp với thể loại, ví dụ: Cảnh Giới, Linh Căn...",
    ...
    properties: {
      storyContext: {
        ...
        properties: {
          worldName: {
            type: Type.STRING,
            description: "Một cái tên độc đáo và hấp dẫn cho thế giới này.",
          },
          genre: {
            type: Type.STRING,
            description:
          ...
          setting: {
            type: Type.STRING,
            description: "Bối cảnh chi tiết của thế giới.",
          },
        },
      ...
      difficulty: {
        ...
        description: "Độ khó của game.",
      },
      allowAdultContent: {
        type: Type.BOOLEAN,
        description: "Cho phép nội dung người lớn hay không.",
      },
      enableStatsSystem: {
        type: Type.BOOLEAN,
        description: "Bật hay tắt hệ thống chỉ số. Luôn đặt là true.",
      },
      enableMilestoneSystem: {
        type: Type.BOOLEAN,
        description: "Bật hay tắt hệ thống cột mốc.",
      },
      initialEntities: {
        type: Type.ARRAY,
        description:
          "Danh sách từ 5 đến 8 thực thể ban đầu trong thế giới (NPC, địa điểm, phe phái...). KHÔNG bao gồm các khái niệm trừu tượng như Lore hay Hệ thống sức mạnh ở đây.",
        items: entitySchema,
      },
    },
    required: [
...
export const getGenerateWorldFromIdeaPrompt = (
  idea: string,
...
7.  **HỆ THỐNG CHỈ SỐ (CHI TIẾT):**
    ...
    c. Với MỖI chỉ số, hãy cung cấp một \`description\` (mô tả) ngắn gọn về công dụng của nó.
    d. Với MỖI chỉ số, bạn BẮT BUỘC phải xác định \`hasLimit\`. Đặt \`hasLimit: true\` cho các chỉ số dạng thanh tài nguyên (Máu, Năng lượng). Đặt \`hasLimit: false\` cho các chỉ số thuộc tính (Sức mạnh, Trí tuệ). Nếu \`hasLimit: false\`, hãy đặt \`maxValue\` bằng \`value\`.
...
export const getGenerateFanfictionWorldPrompt = (
  idea: string,
  generateMilestones: boolean,
  backgroundKnowledge?: { name: string; content: string }[],
) => {
...
export const getGenerateEntityInfoOnTheFlyPrompt = (
  ...
  const schema = {
    ...
    properties: {
      name: {
        type: Type.STRING,
        description: "Tên chính xác của thực thể được cung cấp.",
      },
      type: {
        type: Type.STRING,
      ...
      personality: {
        type: Type.STRING,
        description:
          "Mô tả RẤT ngắn gọn tính cách (1 câu) (chỉ dành cho NPC, để trống cho các loại khác).",
      },
      description: {
        type: Type.STRING,
        description:
      ...
      tags: {
        type: Type.ARRAY,
        items: { type: Type.STRING },
        description: "Một danh sách các tags mô tả ngắn gọn.",
      },
    },
    required: ["name", "type", "description"],
  };
  ...
  if (preDetectedType) {
    instructions =
      `Hệ thống đã tự động xác định loại cốt lõi (type) của thực thể này là "${preDetectedType}". Bạn KHÔNG ĐƯỢC thay đổi trường 'type'.\n` +
      (preDetectedCategory
        ? `Phân loại chi tiết (customCategory) được gợi ý là "${preDetectedCategory}". Bạn có thể tinh chỉnh nó nếu cần.\n`
        : "") +
