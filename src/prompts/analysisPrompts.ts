...
export const getGenerateSummaryPrompt = (turns: GameTurn[]): string => {
  ...
  const historyText = turns
    ...
    .join("\n\n");
  return `Bạn là một AI trợ lý ghi chép. Dựa vào đoạn hội thoại và diễn biến sau, hãy viết một đoạn tóm tắt ngắn gọn (3-4 câu) về các sự kiện chính, các nhân vật mới xuất hiện, và các thông tin quan trọng đã được tiết lộ. Tóm tắt này sẽ được dùng làm ký ức dài hạn. Hãy trả về phần tóm tắt VÀ CHỈ PHẦN TÓM TẮT!, KHÔNG GHI THÊM BẤT KỲ THỨ GÌ KHÁC.\n\n--- LỊCH SỬ CẦN TÓM TẮT ---\n${historyText}`;
};

export const getSummarizeNpcDossierPrompt = (
  npcName: string,
...
export const getRetrieveRelevantSummariesPrompt = (
  context: string,
  allSummaries: string[],
  topK: number,
) => {
...
export const getRetrieveRelevantKnowledgePrompt = (
  context: string,
  detailFiles: { name: string; content: string }[],
  topK: number,
) => {
  const schema = {
    type: Type.OBJECT,
...
Chỉ trả về tên tệp chính xác. Nếu không có gì liên quan, trả về mảng trống.`;
  ...
  const smallAnalyticalConfig: Partial<AiPerformanceSettings> = {
...
export const getDynamicStateUpdatePrompt = (
  ...
  const questSchema = {
    ...
    properties: {
      name: { type: Type.STRING },
      description: { type: Type.STRING },
      status: { type: Type.STRING, enum: ["đang tiến hành", "hoàn thành"] },
    },
  ...
  const schema = {
    ...
    properties: {
      ...
      updatedCompanions: {
        type: Type.ARRAY,
        description: "Danh sách đồng hành đã được cập nhật hoặc mới xuất hiện.",
        items: companionSchema,
      },
      updatedQuests: {
        type: Type.ARRAY,
        description: "Danh sách nhiệm vụ đã được cập nhật hoặc mới xuất hiện.",
        items: questSchema,
      },
      updatedStats: {
        type: Type.ARRAY,
...
- **Chỉ số (\`updatedStats\`):** Có thay đổi nào về chỉ số của nhân vật (Sinh lực, Thể lực...) không? Nếu có, trả về TOÀN BỘ danh sách chỉ số đã cập nhật.`;
  const prompt = createUpdatePrompt(
    lastNarration,
    { inventory, playerStatus, companions, quests, stats: character.stats },
    instructions,
  );
  return { prompt, schema };
};
...
export const getEncyclopediaUpdatePrompt = (
  ...
  const npcSchema = {
    ...
    properties: {
      ...
      description: { type: Type.STRING },
      personality: { type: Type.STRING },
      thoughtsOnPlayer: { type: Type.STRING },
    },
  ...
  const schema = {
    ...
    properties: {
      updatedEncounteredNPCs: {
        type: Type.ARRAY,
        description: "Danh sách NPC đã được cập nhật hoặc mới xuất hiện.",
        items: npcSchema,
      },
      updatedEncounteredFactions: {
        type: Type.ARRAY,
        description: "Danh sách phe phái đã được cập nhật hoặc mới xuất hiện.",
        items: factionSchema,
      },
      updatedDiscoveredEntities: {
        type: Type.ARRAY,
        description:
          "Danh sách thực thể khác (địa điểm, lore...) đã được cập nhật hoặc mới xuất hiện.",
        items: entitySchema,
      },
    },
  };
...
export const getCharacterStateUpdatePrompt = (
  ...
  const reputationChangeSchema = {
    ...
    properties: {
      score: {
        type: Type.NUMBER,
        description: "Số điểm thay đổi (có thể là số dương hoặc âm).",
      },
      reason: {
        type: Type.STRING,
        description: "Lý do ngắn gọn cho sự thay đổi danh vọng.",
      },
    },
  };
  const schema = {
    ...
    properties: {
      ...
      updatedSkills: {
        type: Type.ARRAY,
        description: "Danh sách kỹ năng đã được cập nhật hoặc mới học được.",
        items: skillSchema,
      },
      newMemories: {
        type: Type.ARRAY,
  ...
  const schema = {
    ...
    properties: {
      normalizationMappings: {
        type: Type.ARRAY,
        description:
          "Một danh sách các đối tượng ánh xạ, mỗi đối tượng chứa category cũ và category mới đã được chuẩn hóa. Chỉ bao gồm các category cần thay đổi.",
        items: {
          type: Type.OBJECT,
          properties: {
            oldCategory: {
              type: Type.STRING,
              description: "Tên category cũ cần được thay đổi.",
            },
            newCategory: {
              type: Type.STRING,
              description: "Tên category mới, đã được chuẩn hóa.",
            },
          },
...
export const getDistillKnowledgePrompt = (
  storyIdea: string,
  chunk: string,
  isFinalReduce: boolean = false,
) => {
  ...
  const schema = {
    ...
    properties: {
      deduplicationPairs: {
        type: Type.ARRAY,
        description:
          "Một danh sách các cặp đối tượng, mỗi đối tượng chứa ID của mục cần XÓA và ID của mục cần GIỮ LẠI. Chỉ bao gồm các mục trùng lặp.",
        items: {
          type: Type.OBJECT,
          properties: {
            idToDelete: {
              type: Type.STRING,
              description: "ID của mục cần XÓA.",
            },
            idToKeep: {
              type: Type.STRING,
              description: "ID của mục cần GIỮ LẠI.",
            },
          },
...
export const getPiggybackAnalysisPrompt = (
  ...
  const graphNodeSchema = {
    ...
    properties: {
      id: {
        type: Type.STRING,
        description: "Tên thực thể (Name). Phải là duy nhất.",
      },
      type: {
        type: Type.STRING,
        enum: ["NPC", "Location", "Item", "Event", "Faction", "Concept"],
      },
  ...
  const eqUpdateSchema = {
    ...
    properties: {
      ...
      emotion: {
        type: Type.STRING,
        description: "Trạng thái cảm xúc (Vui, Buồn, Giận...).",
      },
  ...
  const schema = {
    ...
    properties: {
      ...
      eqUpdates: {
        type: Type.ARRAY,
        items: eqUpdateSchema,
        description: "Danh sách cập nhật cảm xúc của các NPC trong cảnh.",
      },
    },
  };
