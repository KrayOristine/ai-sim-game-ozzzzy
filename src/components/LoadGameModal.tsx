...
const LoadGameModal: React.FC<LoadGameModalProps> = ({ isOpen, onClose, onLoad }) => {
  ...
  const [confirmDeleteState, setConfirmDeleteState] = useState<{ isOpen: boolean; saveId: number | null }>({ isOpen: false, saveId: null });
                    ...
                    <div>
                        ...
                        <div className="space-y-3">
                            {manualSaves.map(save => (
                                <SaveItem key={save.saveId} save={save} onLoad={handleLoad} onDownload={fileService.saveGameStateToFile} onDelete={initiateDelete} />
                            ))}
                        </div>
                    </div>
                )}
