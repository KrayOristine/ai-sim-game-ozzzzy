        ...
        <ul className="space-y-2 text-sm">
            {statuses.map((status, index) => (
                <li key={index} className="flex items-center justify-between gap-2 p-2 rounded hover:bg-slate-700/50 bg-slate-900/50">
                    ...
                    <button onClick={() => onDelete(status.name)} className="p-1 text-slate-400 hover:text-red-400 transition-opacity shrink-0" title={`Xóa trạng thái ${status.name}`}>
        ...
        <ul className="space-y-2 text-sm">
            {activeQuests.map((quest, index) => (
                <li key={index} className="group flex items-center justify-between gap-2 p-2 rounded hover:bg-slate-700/50 bg-slate-900/50">
                    ...
                    <button onClick={() => onDelete(quest.name)} className="p-1 text-slate-400 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                        <Icon name="trash" className="w-4 h-4"/>
                    </button>
                </li>
            ))}
