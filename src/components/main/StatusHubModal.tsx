      ...
      {statuses.map((status, index) => (
        ...
        >
          ...
          <button
            ...
            className="p-1 text-slate-400 hover:text-red-400 transition-opacity shrink-0"
      ...
      {activeQuests.map((quest, index) => (
        ...
        >
          ...
          <button
            ...
            className="p-1 text-slate-400 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity shrink-0"
          >
            <Icon name="trash" className="w-4 h-4" />
          </button>
        </li>
...
const StatusHubModal: React.FC<StatusHubModalProps> = ({
  ...
  const activeQuestsCount = (quests || []).filter(
    (q) => !q.status || q.status === "đang tiến hành",
  ).length;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="border-b border-slate-700 mb-4 shrink-0">
          <nav className="flex">
            <button
              className={tabButtonClasses(activeTab === "status", "cyan")}
              onClick={() => setActiveTab("status")}
            >
              Trạng Thái ({statuses.length})
            </button>
            <button
              className={tabButtonClasses(activeTab === "companions", "green")}
              onClick={() => setActiveTab("companions")}
            >
              Đồng Hành ({companions.length})
            </button>
            <button
              className={tabButtonClasses(activeTab === "quests", "blue")}
