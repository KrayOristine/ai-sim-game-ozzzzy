...
const isKnowledgeItem = (item: AllEntities): item is KnowledgeFile =>
  "content" in item && !("description" in item);
...
const TabButton: React.FC<{
  ...
  iconName: any;
}> = ({ active, onClick, children, iconName }) => (
  <button
    onClick={onClick}
  ...
  >
    <Icon name={iconName} className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" />
    <span>{children}</span>
  </button>
);
...
export const EncyclopediaModal: React.FC<EncyclopediaModalProps> = ({
  ...
  const { processedData, dynamicCategories } = useMemo(() => {
    ...
    const allEntitiesFromState = (
      ...
      const all = [
        ...
        ...(state.encounteredFactions || []).map((e) => ({
          ...e,
    ...
    const baseData: Record<string, AllEntities[]> & {
  ...
  const handleSaveEdit = () => {
    ...
    const updatedData = {
      ...editFormData,
      tags:
        typeof editFormData.tags === "string"
          ? editFormData.tags
              .split(",")
              .map((t: string) => t.trim())
              .filter(Boolean)
    ...
    try {
      ...
      setGameState((prev) => {
        ...
        return {
          ...
          discoveredEntities: mergeAndDeduplicate(
          ...
          worldConfig: {
            ...prev.worldConfig,
            initialEntities: mergeAndDeduplicate(
              prev.worldConfig.initialEntities,
              importedData.initialEntities,
            ),
          },
      ...
      setNotification({
        isOpen: true,
        title: "Thành công",
        messages: ["Đã nhập và hợp nhất dữ liệu Bách khoa thành công."],
      });
    } catch (e) {
  ...
  const initiateDeleteCategory = (categoryName: string) => {
    setConfirmState({
      isOpen: true,
  ...
  const executeDeleteCategory = () => {
    const categoryName = confirmState.data;
    setGameState((prev) => {
      const currentCats = prev.customCategories || [];
      return {
        ...prev,
  ...
  const executeAiOptimize = async () => {
    ...
    setNotification({
      isOpen: true,
      title: "Đang xử lý...",
      messages: ["Bắt đầu quá trình chuẩn hóa Bách Khoa Toàn Thư..."],
    });
    ...
    try {
      ...
      setNotification({
        isOpen: true,
        title: "Đang xử lý...",
        messages: ["Bước 1/2: Đang yêu cầu AI chuẩn hóa danh mục..."],
      });

      const allEntitiesForCat = Object.values(processedData).flat() as {
        name: string;
      ...
      if (normalizationMappings.length === 0) {
        setNotification({
          ...
          title: "Thông báo",
          messages: [
            "Bước 1/2: AI không tìm thấy danh mục nào cần chuẩn hóa.",
            "Chuyển sang Bước 2...",
          ],
        });
      } else {
        // Chuyển đổi mảng thành map để dễ dàng tra cứu
        const categoryMap = normalizationMappings.reduce(
          (acc, mapping) => {
        ...
        setNotification({
          ...
          title: "Thông báo",
          messages: [
            `Bước 1/2: Đã chuẩn hóa ${normalizationMappings.length} danh mục.`,
            "Chuyển sang Bước 2...",
          ],
        });
      }
      ...
      for (const category in groupedEntities) {
        const entitiesToDedupe = groupedEntities[category].map((e) => ({
          name: e.name,
          id: e.name,
        }));
      ...
      if (Object.keys(finalDeduplicationMap).length === 0) {
        ...
        setGameState((prev) => {
          ...
          allEntities.forEach((item) => {
            if (
              Object.values(finalDeduplicationMap).includes(item.name) ||
              !itemsToDelete.has(item.name)
            ) {
          ...
          for (const nameToDelete in finalDeduplicationMap) {
            ...
            if (itemToDelete && itemToKeep) {
              ...
              const mergedTags = Array.from(
                new Set([
                  ...((itemToKeep as any).tags || []),
                  ...((itemToDelete as any).tags || []),
                ]),
              );
        ...
        setNotification({
          ...
          title: "Hoàn Tất",
          messages: [
            `Quá trình chuẩn hóa hoàn tất. Đã gộp ${Object.keys(finalDeduplicationMap).length} mục trùng lặp.`,
          ],
        });
      }
    } catch (e) {
      ...
      setNotification({
        isOpen: true,
        title: "Lỗi Chuẩn Hóa",
        messages: [`Lỗi khi chuẩn hóa bằng AI: ${error}`],
      });
    } finally {
  ...
  const handleConfirmAction = () => {
    if (confirmState.type === "delete_category") {
      executeDeleteCategory();
    } else if (confirmState.type === "ai_optimize") {
      executeAiOptimize();
    }
  ...
  const handleGenerateCodex = async () => {
    ...
    try {
      ...
      setNotification({
        isOpen: true,
        title: "Lỗi AI",
        messages: ["Không thể tạo Codex. Vui lòng thử lại."],
      });
    } finally {
    ...
    setGameState((prev) => {
      ...
      if (type === "Item") {
        ...
        if (owner.isPlayer) {
          ...
          if (operation === "update") {
            const idx = newState.character.skills.findIndex(
              (s: any) => s.name.toLowerCase() === targetName.toLowerCase(),
            );
            if (idx > -1) {
              newState.character.skills[idx] = {
                ...newState.character.skills[idx],
                ...newSkill,
              };
            } else {
  ...
  const MainViewTab: React.FC<{
    view: "browse" | "analyze" | "manage";
    label: string;
    icon: any;
  }> = ({ view, label, icon }) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex px-4 pt-2 border-b border-slate-700 shrink-0">
            ...
            <div className="w-1/4 xl:w-1/5 bg-slate-800/50 p-3 shrink-0 flex flex-col">
              ...
              <div className="grow overflow-y-auto pr-2">
                <div className="space-y-2">
                  {fixedTabsConfig.map(
                    (tab) =>
                      processedData[tab.key] &&
                      processedData[tab.key].length > 0 && (
                        <TabButton
                          key={tab.key}
                    ...
                    {filteredList.map((item, index) => (
                      <li key={index}>
                        <button
                          ...
                          className={`cursor-pointer w-full text-left p-2 rounded-md transition-colors ${activeItem?.name === item.name ? "bg-purple-600/30" : "hover:bg-slate-700/50"}`}
                        >
                          <div className="flex justify-between items-center">
                            <p
                              className={`font-semibold truncate ${(item as Quest)?.status === "hoàn thành" ? "text-slate-500 line-through" : "text-slate-100"}`}
                            >
                              {item.name}
                            </p>
                            {activeTab === "quests" &&
                ...
                <div>
                  ...
                  {activeTab === "quests" && (activeItem as Quest).status && (
                    <span
                      className={`text-sm font-semibold px-3 py-1 rounded-full mb-4 inline-block ${
                        (activeItem as Quest).status === "hoàn thành"
                  ...
                  <div className="mb-4">
                    <p className="text-slate-300 whitespace-pre-wrap leading-relaxed">
                      {isKnowledgeItem(activeItem)
                        ? activeItem.content
                        : (activeItem as any).description || "Chưa có mô tả."}
                    </p>
                  </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-slate-900/50 p-4 rounded-lg text-center">
                  ...
                  <p className="text-3xl font-bold text-purple-400">
                    {analysisStats.avgDescLength}
                  </p>
                </div>
                <div className="bg-slate-900/50 p-4 rounded-lg text-center">
                  <p className="text-sm text-slate-400 mb-1">Tag phổ biến</p>
                  <p className="text-xs font-semibold text-orange-400 truncate pt-2">
                    {analysisStats.popularTags || "Không có"}
                  </p>
                </div>
              </div>
              ...
              gameState.customCategories.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {gameState.customCategories.map((cat, idx) => (
                    <div
                      key={idx}
                      className="bg-slate-800 text-slate-200 text-sm px-3 py-1.5 rounded-full flex items-center gap-2 border border-slate-600"
                    >
                      <span>{cat}</span>
                      <button
              ...
              <div className="p-2 space-y-4">
                ...
                <div>
                  <textarea
                    ...
                    className="w-full bg-slate-900/70 border border-slate-600 rounded-md px-3 py-2 text-slate-200 focus:ring-2 focus:ring-fuchsia-500 focus:border-fuchsia-500 transition resize-y min-h-20"
                  />
                </div>
                <div className="flex justify-end">
                  <Button
                    onClick={handleGenerateCodex}
                    disabled={isGeneratingCodex || !codexCommand.trim()}
                    variant="special"
                    className="cursor-pointer text_wauto_py1_px3_sm"
                  >
                  ...
                  <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700 animate-fade-in mt-4">
                    ...
                    <div className="space-y-2 text-sm mb-4">
                      <p>
                        ...
                        <span
                          className={
                            codexPreview.operation === "update"
                              ? "text-yellow-400 font-bold"
                              : "text-green-400 font-bold"
                          }
                        >
                          {codexPreview.operation === "update"
                            ? `CẬP NHẬT (${codexPreview.targetName})`
                            : "TẠO MỚI"}
                        </span>
                      </p>
                      <p>
                      ...
                      <p>
                        ...
                        <span className="text-slate-200">
                          {codexPreview.ownerContext.isPlayer
                            ? "Người Chơi"
                            : codexPreview.ownerContext.npcName || "Chưa rõ"}
                        </span>
                      </p>
                      ...
                      {codexPreview.data.details && (
                        <div className="text-xs text-slate-400 mt-2 p-2 bg-slate-900 rounded">
                          {codexPreview.data.details.stats && (
                            <p>Chỉ số: {codexPreview.data.details.stats}</p>
                          )}
                          {codexPreview.data.details.effects && (
                            <p>Hiệu ứng: {codexPreview.data.details.effects}</p>
                          )}
                        </div>
                      )}
                    </div>
                    <div className="flex justify-end gap-3">
                      <button
                        onClick={() => setCodexPreview(null)}
                        className="cursor-pointer text-slate-400 hover:text-white px-3 py-1"
                      >
                        Hủy
                      </button>
                      <Button
                        ...
                        variant="success"
                        className="cursor-pointer w-auto! py-1! px-4! text-sm!"
                      >
                        {codexPreview.operation === "update"
                          ? "Cập Nhật Dữ Liệu"
                          : "Lưu vào Bách Khoa"}
            ...
            <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700">
              ...
              {isAiOptimizing && (
                <p className="text-sm text-slate-400 mt-2 animate-pulse">
                  AI đang xử lý, quá trình này có thể mất một lúc...
                </p>
              )}
            </div>
          </div>
