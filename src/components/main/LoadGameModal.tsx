...
const SaveItem: React.FC<{
  ...
  onDownload: (save: SaveSlot) => void;
  onDelete: (id: number) => void;
}> = ({ save, onLoad, onDownload, onDelete }) => (
  <div
    key={save.saveId}
  ...
  >
    ...
    <div className="flex items-center gap-2 shrink-0">
      ...
      <button
        ...
        className="cursor-pointer p-2 text-sky-400 hover:bg-sky-500/20 rounded-full transition"
      ...
      <button
        ...
        className="cursor-pointer p-2 text-red-400 hover:bg-red-500/20 rounded-full transition"
        title="Xóa bản lưu"
      >
...
const LoadGameModal: React.FC<LoadGameModalProps> = ({
  ...
  const initiateDelete = (saveId: number) => {
    setConfirmDeleteState({ isOpen: true, saveId });
  };

  const handleConfirmDelete = async () => {
    if (confirmDeleteState.saveId !== null) {
      await gameService.deleteSave(confirmDeleteState.saveId);
      await fetchSaves(); // Refresh list
      setConfirmDeleteState({ isOpen: false, saveId: null });
    }
  };
  ...
  return (
    <>
      <ConfirmationModal
        ...
        onClose={() => setConfirmDeleteState({ isOpen: false, saveId: null })}
        onConfirm={handleConfirmDelete}
        title="Xóa Bản Lưu?"
        message="Bạn có chắc chắn muốn xóa bản lưu này không? Hành động này không thể hoàn tác."
            ...
            {isLoading ? (
              ...
              <div className="space-y-6">
                {manualSaves.length > 0 && (
                  <div>
                    <h3 className="text-lg font-semibold text-blue-300 mb-2 border-b-2 border-blue-500/50 pb-1">
                      Lưu Thủ Công
                    </h3>
                    <div className="space-y-3">
                      {manualSaves.map((save) => (
                        <SaveItem
                          ...
                          save={save}
                          onLoad={handleLoad}
                          onDownload={fileService.saveGameStateToFile}
                ...
                {autoSaves.length > 0 && (
                  <div>
                    <h3 className="text-lg font-semibold text-slate-300 mb-2 border-b-2 border-slate-600/50 pb-1">
                      Lưu Tự Động
                    </h3>
                    <div className="space-y-3">
                      {autoSaves.map((save) => (
