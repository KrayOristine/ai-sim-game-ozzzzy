...
const StatusTooltipWrapper: React.FC<{
  statusName: string;
  statuses: StatusEffect[];
  children: React.ReactNode;
  onClick: () => void;
}> = ({ statusName, statuses, children, onClick }) => {
    ...
    <span className="relative group">
      ...
      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 hidden group-hover:block bg-slate-900 text-white text-xs rounded py-2 px-3 z-10 border border-slate-700 shadow-lg pointer-events-none">
        <p className="font-bold mb-1">
          {status.name} ({status.type === "buff" ? "Tích cực" : "Tiêu cực"})
        </p>
        {status.description}
      </div>
    </span>
...
}> = React.memo(({ content, statuses, onEntityClick }) => {
  const cleanedContent = content.replace(/\s+<\/(entity|important)>/g, "</$1>");
  const parts = cleanedContent
    .split(
      /(<exp>.*?<\/exp>|<thought>.*?<\/thought>|<status>.*?<\/status>|<important>.*?<\/important>|<entity>.*?<\/entity>|".*?")/gs,
    )
    .filter(Boolean);
    ...
    <p className="text-slate-200 whitespace-pre-wrap leading-relaxed">
      {parts.map((part, index) => {
        if (part.startsWith('"') && part.endsWith('"')) {
          return (
            <span key={index} className="text-purple-400 italic">
              {part}
            </span>
          );
        }
        ...
        if (tagMatch) {
          ...
          switch (tagName) {
            ...
            case "thought":
              return (
                <span key={index} className="text-cyan-300 italic">
                  "{innerText}"
                </span>
              );
            case "status":
              return (
                <StatusTooltipWrapper
                  ...
                  statuses={statuses}
                  onClick={() => onEntityClick(innerText)}
                >
                  {innerText}
                </StatusTooltipWrapper>
              );
            case "important":
              return (
                <button
                  ...
                  type="button"
                  onClick={() => onEntityClick(innerText)}
                  className="text-yellow-400 font-semibold cursor-pointer hover:underline focus:outline-none focus:ring-2 focus:ring-yellow-500 rounded-sm bg-transparent p-0 border-0 text-left"
                >
                  {innerText}
                </button>
              );
...
const SuggestionCard: React.FC<{
  ...
  const stripTags = (text: string) =>
    text ? text.replace(/<\/?(entity|important|exp|thought|status)>/g, "") : "";
...
const GameplayScreen: React.FC<GameplayScreenProps> = ({
  ...
  const [entityModalContent, setEntityModalContent] = useState<{
    title: string;
    description: string;
    type: string;
  ...
  const [deletionState, setDeletionState] = useState<{
    isOpen: boolean;
    type: "status" | "entity_hard" | "entity_soft" | "quest" | null;
    data: any;
  ...
  const getTurnsForCurrentPage = () => {
    ...
    const endNarrationIndex = Math.min(
      startNarrationIndex + turnsPerPage,
      narrationIndicesInHistory.length,
    );
  ...
  const handleEntityClick = useCallback(
    async (name: string) => {
      ...
      const status = gameState.playerStatus.find(
        (s) => s.name.toLowerCase().trim() === lowerCaseName,
      );
      if (status)
        found = {
          title: status.name,
      ...
      if (!found) {
        const skill = gameState.character.skills.find(
          (s) => s.name.toLowerCase().trim() === lowerCaseName,
        );
        if (skill)
          found = {
      ...
      if (!found) {
        const companion = gameState.companions.find(
          (c) => c.name.toLowerCase().trim() === lowerCaseName,
        );
        if (companion)
          found = {
            title: companion.name,
            description: `${companion.description}\n\nTính cách: ${companion.personality || "Chưa rõ"}`,
            type: "Đồng hành",
          };
      }
      ...
      if (!found) {
        const npc = gameState.encounteredNPCs.find(
          (n) => n.name.toLowerCase().trim() === lowerCaseName,
        );
        if (npc)
          found = {
            title: npc.name,
            description: `${npc.description}\n\nTính cách: ${npc.personality}\n\nSuy nghĩ về người chơi: "${npc.thoughtsOnPlayer}"`,
            type: "NPC",
          };
      }
      if (!found) {
        const faction = gameState.encounteredFactions.find(
          (f) => f.name.toLowerCase().trim() === lowerCaseName,
        );
        if (faction)
          found = {
            title: faction.name,
            description: faction.description,
            type: "Phe phái/Thế lực",
          };
      }
      if (!found) {
        const discovered = gameState.discoveredEntities?.find(
          (e) => e.name.toLowerCase().trim() === lowerCaseName,
        );
        if (discovered)
          found = {
      ...
      if (!found) {
        const entity = gameState.worldConfig.initialEntities.find(
          (e) => e.name.toLowerCase().trim() === lowerCaseName,
        );
        if (entity)
          found = {
      ...
      if (found) setEntityModalContent(found);
        ...
        setEntityModalContent({
          title: name,
          description: "AI đang tìm kiếm thông tin...",
          type: "Đang tải",
        });
        try {
  ...
  const compressDossiersForTurn = useCallback(
    async (
      ...
      allKnownNpcNames.forEach((npcName) => {
        if (
          lastAction.content.toLowerCase().includes(npcName) ||
          lastNarration.content.toLowerCase().includes(npcName)
        ) {
      ...
      if (involvedNpcs.size > 0) {
        ...
        for (const npcName of involvedNpcs) {
          const originalNpc = [
            ...(stateToCompress.encounteredNPCs || []),
            ...(stateToCompress.companions || []),
  ...
  const handleActionSubmit = useCallback(
    async (actionContent: string) => {
      if (!actionContent.trim() || isLoading) return;

      // Bước 1: Trích xuất thời gian từ input của người chơi bằng code
      const extractedTime: TimePassed = extractTimePassedFromText(
        actionContent,
        gameState.worldTime,
      );

      const newAction: GameTurn = {
        type: "action",
        content: actionContent.trim().replace(/<[^>]*>/g, ""),
      };
      ...
      try {
        ...
        setGameState((prev) => {
          ...
          let updatedState = {
            ...prev,
            history: [...prev.history, narrationTurn],
          };
          ...
          if (involvedNpcs.size > 0) {
            ...
            involvedNpcs.forEach((npcNameKey) => {
              if (!finalState.npcDossiers![npcNameKey]) {
                finalState.npcDossiers![npcNameKey] = {
                  fresh: [],
                  archived: [],
                };
              }
  ...
  const startGame = useCallback(async () => {
    ...
    setNotificationModal((prev) => ({ ...prev, isOpen: false }));
    try {
      const { narration, tags, worldSim, thinking } = await aiService.startGame(
  ...
  useEffect(() => {
    if (gameState.history.length === 0) startGame();
  }, [gameState.history.length, startGame]);

  useEffect(() => {
    if (
      gameState.history.length > 0 &&
      (!gameState.reputationTiers || gameState.reputationTiers.length !== 5)
    ) {
      const fetchTiers = async () => {
        try {
          const tiers = await aiService.generateReputationTiers(
          ...
          setGameState((prev) => {
            ...
            const { finalState } = dispatchTags(newState, [
              { tagName: "REPUTATION_CHANGED", params: { score: 0 } },
            ]);
            gameService.saveGame(finalState, "auto");
            return finalState;
          });
    ...
    gameState.reputation.score,
    gameState.reputationTiers,
    gameState.worldConfig.storyContext.genre,
  ]);
  ...
  const handleConfirmUndo = async () => {
    if (
      ...
      gameState.history[gameState.history.length - 1].type !== "narration"
    ) {
  ...
  const handleSaveTemporaryRules = async (newRules: TemporaryRule[]) => {
    const updatedGameState = {
      ...gameState,
      worldConfig: { ...gameState.worldConfig, temporaryRules: newRules },
    };
    setGameState(updatedGameState);
    await gameService.saveGame(updatedGameState, "auto");
  ...
  const handleDeleteStatus = (statusName: string) => {
    setDeletionState({ isOpen: true, type: "status", data: statusName });
  };

  // Logic xóa "cứng" (Initiate)
  ...
  // Update handleQuestClick to pass the whole quest object
  ...
  // Logic xóa Quest (Initiate)
  const handleDeleteQuest = useCallback((questName: string) => {
    setDeletionState({ isOpen: true, type: "quest", data: questName });
  }, []);

  // Execution Function for all deletions
  const executeDeletion = useCallback(() => {
    const { type, data } = deletionState;
    if (!type || !data) return;
    ...
    setGameState((prev) => {
      ...
      if (type === "status") {
        ...
        newState.companions = (newState.companions || []).filter(
          (c: Companion) => c.name.toLowerCase() !== nameToDelete,
        );
  ...
  const handlePageChange = (updater: (p: number) => number) => {
    setCurrentPage((prev) => {
      ...
      if (newPage !== prev) setIsPaginating(true);
      return newPage;
    });
  };

  const characterPersonality =
    gameState.character.personality === "Tuỳ chỉnh"
      ...
      <div className="shrink-0 space-y-4">
        ...
        <div className="bg-slate-900/50 p-3 rounded-lg">
          <div className="flex items-center gap-2 text-orange-400 font-semibold text-sm mb-2">
            <Icon name="reputation" className="w-5 h-5" />
            Danh Vọng
          </div>
                    ...
                    <>
                      <div className="flex justify-between items-center mb-0.5">
                        <span
                          ...
                          title={stat.description || stat.name}
                        >
                          {stat.name}
                        </span>
                      ...
                      <div className="w-full bg-slate-700 rounded-full h-1.5 overflow-hidden">
                        <div
                          className={`h-1.5 rounded-full transition-all duration-500 ${
                            (stat.value / stat.maxValue) * 100 > 50
                              ? "bg-green-500"
                              : (stat.value / stat.maxValue) * 100 > 20
                                ? "bg-yellow-500"
                                : "bg-red-500"
                          }`}
                          style={{
                            width: `${(stat.value / Math.max(1, stat.maxValue)) * 100}%`,
                          }}
                        ></div>
                      </div>
                    </>
      ...
      <div className="grow space-y-2">
        ...
        >
          <span className="flex items-center">
            <Icon name="info" className="w-5 h-5 mr-3 text-pink-400" />
            Túi Đồ & Thông Tin
          </span>
          {gameState.inventory.length > 0 && (
            <span className="text-xs bg-slate-600 text-white font-bold px-1.5 py-0.5 rounded-full">
        ...
        <button
          onClick={() => {
            setIsStatusHubOpen(true);
            setIsSidePanelOpen(false);
          }}
          className="cursor-pointer w-full flex items-center justify-between px-3 py-3 text-sm text-left rounded-md hover:bg-slate-700 transition"
        >
          <span className="flex items-center">
            <Icon name="hub" className="w-5 h-5 mr-3 text-cyan-400" />
            Trạng Thái & Nhiệm Vụ
          </span>
          <div className="flex gap-2">
            {gameState.playerStatus.length > 0 && (
              <span className="text-xs bg-red-500/80 text-white font-bold px-1.5 py-0.5 rounded-full">
                {gameState.playerStatus.length}
              </span>
            )}
            {activeQuests.length > 0 && (
              <span className="text-xs bg-blue-500/80 text-white font-bold px-1.5 py-0.5 rounded-full">
                {activeQuests.length}
              </span>
            )}
          </div>
        ...
        <button
          onClick={() => {
            setIsEncyclopediaModalOpen(true);
            setIsSidePanelOpen(false);
          }}
          className="cursor-pointer w-full flex items-center justify-between px-3 py-3 text-sm text-left rounded-md hover:bg-slate-700 transition"
        >
        ...
        >
          <span className="flex items-center">
            <Icon name="memory" className="w-5 h-5 mr-3 text-purple-400" />
            Ký Ức
          </span>
          {gameState.memories.length + gameState.summaries.length > 0 && (
            <span className="text-xs bg-slate-600 text-white font-bold px-1.5 py-0.5 rounded-full">
              {gameState.memories.length + gameState.summaries.length}
            </span>
          )}
        </button>
        <button
          onClick={() => {
            setIsTempRulesModalOpen(true);
            setIsSidePanelOpen(false);
          }}
          className="cursor-pointer w-full flex items-center justify-between px-3 py-3 text-sm text-left rounded-md hover:bg-slate-700 transition"
        >
          <span className="flex items-center">
            <Icon name="rules" className="w-5 h-5 mr-3 text-blue-400" />
            Luật Tạm Thời / Ghi Chú
          </span>
        </button>
      </div>
      ...
      <div className="shrink-0 pt-4 border-t border-slate-700">
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            disabled={isSaving}
            className="cursor-pointer flex items-center justify-center gap-2 px-3 py-2 text-xs font-semibold text-green-300 bg-green-900/40 hover:bg-green-800/60 rounded-lg transition disabled:opacity-50"
          >
          ...
          <button
            ...
            className="cursor-pointer flex items-center justify-center gap-2 px-3 py-2 text-xs font-semibold text-yellow-300 bg-yellow-900/40 hover:bg-yellow-800/60 rounded-lg transition disabled:opacity-50"
          >
          ...
          <button
            ...
            className="cursor-pointer flex items-center justify-center gap-2 px-3 py-2 text-xs font-semibold text-pink-300 bg-pink-900/40 hover:bg-pink-800/60 rounded-lg transition"
          >
          ...
          <button
            ...
            className="cursor-pointer flex items-center justify-center gap-2 px-3 py-2 text-xs font-semibold text-slate-300 bg-slate-700/80 hover:bg-slate-700 rounded-lg transition"
          >
  ...
  return (
    <>
      <ConfirmationModal
        ...
        title={
          deletionState.type === "status"
            ? "Xóa Trạng Thái?"
            : deletionState.type === "quest"
              ? "Từ Bỏ Nhiệm Vụ?"
              : "Xóa Thực Thể?"
        }
        message={
          deletionState.type === "status"
            ? `Bạn có chắc muốn xóa trạng thái "${deletionState.data}" không?`
            : deletionState.type === "quest"
              ? `Bạn có chắc muốn từ bỏ nhiệm vụ "${deletionState.data}" không?`
              : deletionState.type === "entity_hard"
                ? `Bạn có chắc muốn xóa "${deletionState.data?.name}" không? Thao tác này sẽ xóa mục này khỏi mọi nơi trong game (túi đồ, kỹ năng, bách khoa...).`
                : `Hành động này chỉ xóa thông tin trong Bách Khoa, không ảnh hưởng đến vật phẩm/kỹ năng của nhân vật. Bạn có chắc muốn xóa '${deletionState.data?.name}' không?`
        }
      />
      <NotificationModal
      ...
      <EncyclopediaModal
        ...
        onDeleteEntity={handleDeleteFromEncyclopedia}
      />
      <EntityInfoModal
      ...
      <StatusHubModal
        ...
        onSelectStatus={handleEntityClick}
        onDeleteStatus={handleDeleteStatus}
        onSelectCompanion={handleCompanionClick}
        onSelectQuest={handleQuestClick}
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-2xl p-6 w-full max-w-md relative animate-fade-in-up">
            ...
            <div className="flex justify-end gap-4">
              ...
              <button
                ...
                className="text-slate-400 hover:text-white transition px-4 py-2 rounded-md"
              >
                Hủy
              </button>{" "}
            </div>{" "}
      ...
      {showRestartConfirm && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-60 p-4">
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-2xl p-6 w-full max-w-md relative animate-fade-in-up">
            ...
            <p className="text-slate-300 mb-6">
              Bạn có muốn lưu tiến trình hiện tại trước khi bắt đầu lại không?
            </p>{" "}
            <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
              {" "}
              <button
                onClick={() => setShowRestartConfirm(false)}
                className="w-full sm:w-auto text-slate-300 hover:text-white transition px-4 py-2 rounded-md text-center bg-slate-700/50 hover:bg-slate-700"
              >
                Hủy
              </button>{" "}
              <Button
                onClick={handleRestartWithoutSaving}
                variant="warning"
                className="w-full! sm:w-auto! py-2! px-4!"
              >
                Không Lưu & Bắt Đầu Lại
              </Button>{" "}
              <Button
      ...
      <div className="lg:flex h-screen bg-slate-900 text-slate-200 font-sans lg:gap-4">
        ...
        <div className="flex-1 flex flex-col h-full p-2 sm:p-4 gap-2 sm:gap-4 lg:p-4 lg:pr-0">
          <header className="shrink-0 bg-slate-800/50 p-2 rounded-lg">
            <div className="flex justify-between items-center">
              <button
                onClick={() => setShowExitConfirm(true)}
                className="cursor-pointer flex items-center gap-2 px-4 py-2 text-sm font-semibold text-slate-200 bg-slate-700/80 hover:bg-slate-700 rounded-lg transition"
              >
                <Icon name="back" className="w-4 h-4" />
                <span className="hidden sm:inline">Về Trang Chủ</span>
              </button>
              <div className="text-center">
                <h1 className="text-base sm:text-lg font-bold text-slate-100 truncate max-w-37.5 sm:max-w-87.5">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="lg:hidden z-60">
                  <button
                    ...
                    className="p-2 text-slate-300 hover:bg-slate-700 rounded-full transition"
                  >
          ...
          <main className="flex-1 flex flex-col bg-slate-800/50 rounded-lg p-2 sm:p-4 overflow-hidden relative">
            ...
            <div
              ref={logContainerRef}
              onClick={handleNarrationContainerClick}
              className={`flex-1 overflow-y-auto mb-4 pr-2 space-y-6 cursor-pointer`}
            >
            ...
            <div className="absolute bottom-24 right-4 z-20 flex flex-col gap-2">
              {showScrollUp && (
                <button
                  ...
                  className="bg-slate-700/80 hover:bg-slate-600/90 backdrop-blur-sm text-white p-2 rounded-full transition-opacity duration-300 animate-fade-in"
            ...
            <div className="shrink-0 mt-auto bg-slate-900/50 rounded-lg p-3 sm:p-4">
                ...
                <div className="mb-4">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    >
                      ...
                      <Icon
                        name={showSuggestions ? "arrowUp" : "arrowDown"}
                        className={`w-3 h-3 transition-transform duration-300 ${showSuggestions ? "rotate-180" : ""}`}
                      />{" "}
                    </button>{" "}
                  </div>
                  <div
                    className={`grid transition-all duration-500 ease-in-out ${showSuggestions ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}
                  >
                    <div className="overflow-hidden">
                      {" "}
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 max-h-72 overflow-y-auto pr-2 pb-2">
                        {gameState.suggestions.map((s, i) => (
                          <SuggestionCard
                            key={i}
                            index={i}
                            suggestion={s}
                            onSelect={handleActionSubmit}
                          />
                        ))}
                      </div>{" "}
                    </div>
              ...
              <div>
                ...
                <div className="flex items-stretch gap-2 sm:gap-3">
                  ...
                  <Button
                    ...
                    disabled={isLoading}
                    variant="primary"
                    className="w-auto! py-3! px-4! sm:px-6! self-stretch text-base! cursor-pointer"
                  >
                    {isTurnLoading ? (
                      <svg
              ...
              <div className="flex items-center justify-center gap-2 mt-2 shrink-0">
                ...
                <button
                  ...
                  className="px-2 py-1 rounded-md bg-slate-700 hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed transition"
                >
                  <Icon name="arrowDown" className="w-4 h-4 -rotate-90" />
                </button>{" "}
              </div>
            </div>
          </main>
        ...
        <aside className="w-[320px] xl:w-87.5 shrink-0 h-full hidden lg:flex flex-col p-4 pl-0">
          <DashboardSidebar />
        </aside>
      </div>
      ...
      {/* Mobile Side Panel */}
      <div
        className={`fixed inset-0 z-40 transition-opacity duration-300 lg:hidden ${isSidePanelOpen ? "bg-black/60 backdrop-blur-sm" : "pointer-events-none bg-transparent"}`}
        onClick={() => setIsSidePanelOpen(false)}
      ></div>
      <div
        ref={menuRef}
        className={`fixed top-0 right-0 h-full w-4/5 max-w-xs bg-slate-800/95 backdrop-blur-lg border-l border-slate-700 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out lg:hidden flex flex-col ${isSidePanelOpen ? "translate-x-0" : "translate-x-full"}`}
      >
