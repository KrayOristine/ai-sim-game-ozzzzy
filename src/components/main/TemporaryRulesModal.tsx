...
const TemporaryRulesModal: React.FC<TemporaryRulesModalProps> = ({
  isOpen,
  onClose,
  onSave,
  initialRules,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="bg-slate-900/70 border border-blue-900/50 rounded-md p-3 mb-4 text-xs text-slate-400 shrink-0">
          ...
          {rules.map((rule, index) => (
            ...
            >
              ...
              <input
                ...
                className={`grow bg-slate-800 border border-slate-600 rounded-md px-3 py-1.5 text-slate-200 transition focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${!rule.enabled ? "text-slate-500 line-through" : ""}`}
              />
              <button
                onClick={() => handleRemoveRule(index)}
                className="p-2 text-red-400 hover:bg-red-500/20 rounded-full transition shrink-0"
              >
        ...
        <div className="mt-4 shrink-0 flex justify-between items-center">
          ...
          <Button
            onClick={handleCloseAndSave}
            variant="primary"
            className="cursor-pointer w-auto! py-2! px-5! text-sm!"
          >
            Đóng & Lưu
          </Button>
        </div>
