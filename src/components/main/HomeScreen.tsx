  ...
  useEffect(() => {
    const initializeApp = async () => {
      ...
      try {
        ...
        alert(
          "Đã xảy ra lỗi khi cố gắng di chuyển các file lưu cũ. Dữ liệu của bạn vẫn an toàn trong bộ nhớ cũ.",
        );
      } finally {
    ...
    if (file) {
      try {
        ...
        if (parsedJson.worldConfig && Array.isArray(parsedJson.history)) {
          ...
          throw new Error(
            "Tệp JSON không có cấu trúc hợp lệ cho thiết lập thế giới hoặc file game đã lưu.",
          );
        }
      } catch (error) {
        alert(
  ...
  return (
    <>
      ...
      <NotificationModal
        ...
        messages={[
          "Chức năng này hiện chưa hoàn thiện và sẽ sớm được cập nhật trong các phiên bản sau. Cảm ơn bạn đã thông cảm!",
        ]}
      />
      <div className="flex flex-col items-center justify-center min-h-screen p-4">
        <div className="text-center mb-10">
          <h1 className="title-main">Nhập Vai A.I Simulator</h1>
          <p className="subtitle-main">Kiến tạo thế giới của riêng bạn</p>
        </div>
        ...
        <div className="w-full max-w-sm space-y-4">
          ...
          />
          <Button
            onClick={onNavigateToFandomGenesis}
