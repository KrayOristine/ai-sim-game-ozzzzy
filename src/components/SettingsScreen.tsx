...
import { HARM_CATEGORIES, HARM_BLOCK_THRESHOLDS, DEFAULT_SAFETY_SETTINGS, DEFAULT_AI_PERFORMANCE_SETTINGS } from '#/const';
...
const SettingsScreen: React.FC<SettingsScreenProps> = ({ onBack }) => {
  ...
  const getInputClass = (status: ValidationStatus = 'idle') => {
    switch(status) {
        case 'valid': return 'input-valid';
        case 'invalid': return 'input-invalid';
        case 'rate_limited': return 'input-rate-limited';
        case 'loading': return 'input-loading';
  ...
  const isHighEndModel = settings.aiPerformanceSettings.selectedModel?.includes('gemini-2.5-pro') || settings.aiPerformanceSettings.selectedModel?.includes('gemini-3.1-pro');
    ...
    <div className="settings-container">
      ...
      <Accordion title="Thiết lập API Key (Tự động lưu)" icon={<Icon name="key"/>} borderColorClass='border-cyan-500' titleClassName='text-cyan-400'>
          ...
          <div className="key-list">
              {settings.apiKeyConfig.keys.map((key, index) => (
                  <div key={index} className="input-row">
                      ...
                      <button onClick={() => removeKeyInput(index)} className="btn-delete" disabled={settings.apiKeyConfig.keys.length <= 1 && settings.apiKeyConfig.keys[0] === ''}>
          ...
          <div className="btn-row">
            <Button onClick={addKeyInput} variant="secondary" className="btn-small"><Icon name="plus" className="icon-md-mr"/> Thêm API</Button>
            <Button onClick={handleFileUploadClick} variant="secondary" className="btn-small"><Icon name="upload" className="icon-md-mr"/> Tải lên từ tệp (.txt)</Button>
            <Button onClick={handleTestAllKeys} variant="special" className="btn-small" disabled={isTestingKeys}>
      ...
      <Accordion title="Cài đặt an toàn Gemini API" icon={<Icon name="warning"/>} borderColorClass='border-blue-500' titleClassName='text-blue-400'>
          <p className="text-desc">Cấu hình mức độ chặn nội dung không an toàn. Ảnh hưởng đến tất cả phản hồi AI.</p>
          <div className="info-box">
            <h3 className="heading-section">Chế độ hoạt động kép</h3>
            <div className="text-info">
              <p><strong className="text-strong">Bật lọc an toàn (Chế độ Tiêu chuẩn):</strong> Văn bản của bạn được gửi trực tiếp đến Gemini API và bộ lọc an toàn sẽ hoạt động theo các mức chặn bạn đã định cấu hình bên dưới. Sử dụng chế độ này cho nội dung thông thường.</p>
              <p><strong className="text-strong">Tắt lọc an toàn (Chế độ Vượt qua An toàn):</strong> Để xử lý nội dung nhạy cảm (khi tùy chọn 18+ được bật) và tránh lỗi "prohibited", công cụ sẽ kích hoạt một giao thức đặc biệt. Văn bản đầu vào của bạn sẽ được làm mờ (obfuscated) và AI sẽ được hướng dẫn theo một "Giao thức Nghiên cứu Học thuật" để giải mã và xử lý nội dung gốc một cách trung thực và không bị kiểm duyệt.</p>
            </div>
          </div>
          ...
          <div className={`opacity-disabled ${settings.safetySettings.enabled ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
      ...
      <Accordion title="Hệ Thống Trí Nhớ Nâng Cao (RAG & Tóm Tắt)" icon={<Icon name="memory"/>} borderColorClass='border-purple-500' titleClassName='text-purple-400'>
        <p className="text-desc">Cấu hình cách AI ghi nhớ và truy xuất thông tin dài hạn để chống quá tải token và giúp game chạy được hàng ngàn lượt.</p>
        <div className="space-section">
          <div>
            <label htmlFor="summary-frequency" className="label-form">Tần suất Tóm tắt Tự động (số lượt)</label>
            <input
              type="number"
              ...
              min="5"
            />
            <p className="text-small-desc">Cứ sau mỗi X lượt, AI sẽ tự động tóm tắt các diễn biến vừa qua.</p>
          </div>
          <div>
            <label htmlFor="top-k" className="label-form">Số kết quả RAG (Top K)</label>
            <input
              type="number"
              id="top-k"
              ...
              max="10"
            />
            <p className="text-small-desc">Số lượng ký ức liên quan nhất AI sẽ truy xuất trong mỗi lượt chơi.</p>
          </div>
      ...
      <Accordion title="Cài đặt Hiệu suất AI" icon={<Icon name="difficulty"/>} borderColorClass='border-yellow-500' titleClassName='text-yellow-400'>
        <p className="text-desc">Điều chỉnh các thông số kỹ thuật của AI để cân bằng giữa chất lượng, tốc độ và chi phí. Chỉ dành cho người dùng nâng cao.</p>
        <div className="space-performance">
          <div>
            <label htmlFor="model-select" className="label-form">Model Dẫn Truyện</label>
            <select
              id="model-select"
            ...
            >
              ...
              <option value="gemini-3-flash-preview">Gemini 3.0 Flash (Tốc độ cao - Thông minh hơn - Mới)</option>
              <option value="gemini-3.1-flash-lite-preview">Gemini 3.1 Flash Lite (Siêu tốc - Tiết kiệm - Mới)</option>
            </select>
          ...
          <div className={isHighEndModel ? 'disabled-section' : ''}>
              <div className="slider-section">
                <div className="slider-label">
                  <label htmlFor="max-tokens-input" className="label-form">Độ dài Phản hồi Tối đa (Max Output Tokens)</label>
                  <input
                    type="number"
                    id="max-tokens-input"
              ...
              <div className={`slider-section ${(!isGen3Model && !isHighEndModel) ? 'disabled-section' : ''}`}>
                <div className="slider-label">
                  ...
                  <select
                    ...
                    onChange={(e) => handleAiPerformanceSettingChange('thinkingLevel', e.target.value)}
                    className="select-small"
                    style={{ minWidth: 360 }}
                  >
                    <option value="None">None (tắt hoàn toàn suy nghĩ)</option>
                    <option value="Minimal" disabled>Minimal (tối thiểu, không hỗ trợ)</option>
                    <option value="Low">Low (thấp, tốn thời gian rất ít)</option>
                    <option value="Medium">Medium (vừa phải, không tiêu hao quá nhiều thời gian)</option>
              ...
              <div className="slider-section">
                <div className="slider-label">
                    <label htmlFor="json-buffer-input" className="label-form">Độ dài Bổ sung cho JSON (jsonBuffer)</label>
                     <input
                      type="number"
                ...
                <input
                  ...
                  step="128"
                />
                <p className="text-small-desc">Thêm token dự phòng để đảm bảo AI có đủ không gian cho cấu trúc dữ liệu game (JSON), tránh lỗi. Giá trị này sẽ được cộng thêm vào giới hạn token cuối cùng khi gọi AI. Mặc định: 1024.</p>
              </div>
          </div>
          {isHighEndModel && (
