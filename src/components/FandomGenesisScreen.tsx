...
const FandomGenesisScreen: React.FC<FandomGenesisScreenProps> = ({ onBack }) => {
  ...
  const [confirmDeleteState, setConfirmDeleteState] = useState<{ isOpen: boolean; fileId: number | null }>({ isOpen: false, fileId: null });
              ...
              {generatedResult && (
                  <>
                      ...
                      <p className="text-sm text-amber-300 mt-2 flex items-start">
                          <Icon name="info" className="w-4 h-4 inline mr-2 mt-0.5 shrink-0"/>
                        ...
                        <div key={file.id} className="bg-slate-900/50 p-3 rounded-lg flex items-center justify-between gap-4">
                           {renamingFileId === file.id ? (
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                                ...
                                <button onClick={() => initiateDelete(file.id)} className="p-2 text-red-400 hover:bg-red-500/20 rounded-full transition" title="Xóa tệp">
