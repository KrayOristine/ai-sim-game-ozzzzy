...
const GameplayScreen: React.FC<GameplayScreenProps> = ({ initialGameState, onBack }) => {
  ...
  const [entityModalContent, setEntityModalContent] = useState<{ title: string; description: string; type: string; details?: InitialEntity['details']; questData?: any } | null>(null);
  ...
  const handleEntityClick = useCallback(async (name: string) => {
    ...
    let found: { title: string; description: string; type: string; details?: InitialEntity['details']; questData?: any } | null = null;
    ...
    if (!found) {
        ...
        if(quest) found = { title: quest.name, description: quest.description, type: 'Nhiệm Vụ', questData: quest };
    }
  ...
  const handleDeleteEntity = useCallback((entityToDelete: { name: string }) => {
      setDeletionState({ isOpen: true, type: 'entity_hard', data: entityToDelete });
  }, []);

  // Logic xóa "mềm" (Initiate)
  const handleDeleteFromEncyclopedia = useCallback((entityToDelete: { name: string }) => {
      setDeletionState({ isOpen: true, type: 'entity_soft', data: entityToDelete });
  }, []);
  ...
  const handleQuestClick = useCallback((quest: Quest) => setEntityModalContent({ title: quest.name, description: quest.description, type: 'Nhiệm Vụ', questData: quest }), []);
  ...
  const executeDeletion = useCallback(() => {
      ...
      setGameState(prev => {
          ...
          if (type === 'status') {
              newState.playerStatus = newState.playerStatus.filter((s: StatusEffect) => s.name.trim().toLowerCase() !== data.trim().toLowerCase());
          }
          else if (type === 'quest') {
  ...
  return (
    <>
      <ConfirmationModal
        ...
        title={
            deletionState.type === 'status' ? "Xóa Trạng Thái?" :
            deletionState.type === 'quest' ? "Từ Bỏ Nhiệm Vụ?" :
            "Xóa Thực Thể?"
        }
        message={
            deletionState.type === 'status' ? `Bạn có chắc muốn xóa trạng thái "${deletionState.data}" không?` :
            deletionState.type === 'quest' ? `Bạn có chắc muốn từ bỏ nhiệm vụ "${deletionState.data}" không?` :
            deletionState.type === 'entity_hard' ? `Bạn có chắc muốn xóa "${deletionState.data?.name}" không? Thao tác này sẽ xóa mục này khỏi mọi nơi trong game (túi đồ, kỹ năng, bách khoa...).` :
      ...
      <EntityInfoModal isOpen={!!entityModalContent} onClose={() => setEntityModalContent(null)} title={entityModalContent?.title || null} description={entityModalContent?.description || null} type={entityModalContent?.type || null} details={entityModalContent?.details || undefined} questData={entityModalContent?.questData} />
            ...
            <header className="shrink-0 bg-slate-800/50 p-2 rounded-lg">
              <div className="flex justify-between items-center">
                ...
                <div className="text-center">
                  <h1 className="text-base sm:text-lg font-bold text-slate-100 truncate max-w-37.5 sm:max-w-87.5">{gameState.worldConfig.storyContext.worldName || gameState.worldConfig.storyContext.genre}</h1>
