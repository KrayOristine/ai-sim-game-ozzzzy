 ...
 */
export function processNpcNewOrUpdate(
 ...
 * @returns Một đối tượng chứa trạng thái game mới và các yêu cầu cập nhật vector.
 */
export function processNpcThoughtsUpdate(
  currentState: GameState,
  params: any,
): { newState: GameState; vectorUpdates: VectorUpdate[] } {
  if (!params.name) {
  ...
  if (existingNpcIndex > -1) {
    ...
    if (initialEntityNpc) {
      ...
      console.warn(
        `Thẻ [NPC_UPDATE] được gọi cho NPC chưa tồn tại trong initialEntities: "${sanitizedName}". Tự động tạo mới.`,
      );
 ...
 */
export function processMemoryFlag(
  currentState: GameState,
  params: any,
): { newState: GameState; vectorUpdates: VectorUpdate[] } {
  ...
  if (!npcFound) {
    console.warn(
      `Thẻ [MEM_FLAG] được gọi cho NPC chưa tồn tại: "${sanitizedName}". Tự động tạo mới.`,
    );
    const newNpc: EncounteredNPC = {
      name: sanitizedName,
      description: "Chưa rõ",
 ...
 * @param params
 */
export function processNpcEmotion(
  currentState: GameState,
  params: any,
): { newState: GameState; vectorUpdates: VectorUpdate[] } {
