 ...
 */
export function processQuestUpdate(
  ...
  const newQuestData: Quest = {
    ...
    status:
      params.status === "hoàn thành" || params.status === "thất bại"
        ? params.status
        : "đang tiến hành",
    tags: params.tags
    ...
    currentObjective:
      params.objective ||
      existingQuest?.currentObjective ||
      params.description ||
      "Thực hiện nhiệm vụ",
    logs: logs,
    subTasks:
 ...
 */
export function processQuestLog(
  currentState: GameState,
  params: any,
): { newState: GameState; vectorUpdates: VectorUpdate[] } {
  if (!params.name) {
  ...
  if (params.subTaskComplete && quest.subTasks) {
    ...
    if (hasChange && !params.entry) {
      ...
      if (completedTask) {
        quest.logs = [
          ...(quest.logs || []),
          `${timeStr} - Hoàn thành mục tiêu: ${completedTask.desc}`,
        ];
      }
    }
  }
