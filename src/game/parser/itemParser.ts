 ...
 */
export function processItemAdd(
  ...
  if (params.target && params.target.toLowerCase() !== "player") {
    console.warn(
      `[Code-Security] Bỏ qua thẻ [ITEM_ADD] vì mục tiêu không phải người chơi: target="${params.target}"`,
    );
 ...
 */
export function processItemRemove(
  currentState: GameState,
  params: any,
): { newState: GameState; vectorUpdates: VectorUpdate[] } {
  // BƯỚC 1: Thêm lớp bảo mật Code
  ...
  if (params.target && params.target.toLowerCase() !== "player") {
    console.warn(
      `[Code-Security] Bỏ qua thẻ [ITEM_REMOVE] vì mục tiêu không phải người chơi: target="${params.target}"`,
    );
    return { newState: currentState, vectorUpdates: [] };
  }
