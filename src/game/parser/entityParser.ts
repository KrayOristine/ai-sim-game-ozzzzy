 ...
 */
export function processFactionUpdate(
 ...
 * @returns Một đối tượng chứa trạng thái game mới và các yêu cầu cập nhật vector.
 */
export function processEntityDiscovered(
  currentState: GameState,
  params: any,
  type: "Địa điểm" | "Hệ thống sức mạnh / Lore",
): { newState: GameState; vectorUpdates: VectorUpdate[] } {
  if (!params.name) {
    console.warn(
      `Bỏ qua thẻ [${type === "Địa điểm" ? "LOCATION_DISCOVERED" : "LORE_DISCOVERED"}] không hợp lệ:`,
      params,
    );
    return { newState: currentState, vectorUpdates: [] };
  }
