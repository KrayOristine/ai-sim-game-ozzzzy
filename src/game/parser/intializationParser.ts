 ...
 */
export function processPlayerStatsInit(
 ...
 * @returns Một đối tượng chứa trạng thái game mới và mảng vectorUpdates rỗng.
 */
export function processWorldTimeSet(
  currentState: GameState,
  params: any,
): { newState: GameState; vectorUpdates: VectorUpdate[] } {
 ...
 * @returns Một đối tượng chứa trạng thái game mới và mảng vectorUpdates rỗng.
 */
export function processReputationTiersSet(
  currentState: GameState,
  params: any,
): { newState: GameState; vectorUpdates: VectorUpdate[] } {
  let tiers: string[] = [];
  ...
  if (tiers.length !== 5) {
    console.warn(
      "Thẻ [REPUTATION_TIERS_SET] phải chứa đúng 5 cấp bậc. Sử dụng giá trị mặc định.",
      params,
    );
    return { newState: currentState, vectorUpdates: [] };
  }
