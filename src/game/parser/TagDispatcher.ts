...
export function dispatchTags(
  ...
  tags: ParsedTag[],
): { finalState: GameState; vectorUpdates: VectorUpdate[] } {
    ...
    if (
      ...
      if (itemKeywords.some((kw) => name.includes(kw))) {
        ...
        if (!locationKeywords.some((kw) => name.includes(kw))) {
          console.warn(
            `[Code-Validation] Chuyển thẻ ${newTag.tagName} thành ITEM_ADD vì tên "${newTag.params.name}" chứa từ khóa vật phẩm.`,
          );
    ...
    (acc: { state: GameState; updates: VectorUpdate[] }, tag) => {
      ...
      switch (tag.tagName) {
        ...
        case "QUEST_UPDATE":
          ...
          break;
        case "QUEST_LOG": // Thẻ mới
          processResult = processQuestLog(acc.state, tag.params);
          break;
        case "COMPANION_NEW":
